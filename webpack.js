	...
	module.exports = {
		...
		module: { // блок для конфигурирования loaders
			...
			rules: [ // массив - правила, как работать с модулями (для webpack любой файл - это модуль)
				...
				{
					test: /\.css$/i,
					use: [ // когда используется композиция loaders, они применяются в обратном порядке.
						   // То есть, порядок вызовов loaders имеет значение
						{
							loader: 'style-loader' // этот сработает вторым (подгружает стили, полученные css-loader)
						},
						{
							loader: 'css-loader' // этот сработает первым (работает с css-файлом)
						}
					]
					
					// use: ['style-loader', 'css-loader'] // упрощённая запись loaders без опций
				}
			]
		}
	};
